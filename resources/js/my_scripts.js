...
function loadStatsPage()
{
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
	{
		...
		if(parseInt(oppscore) > parseInt(homescore))
		{
...
function loadPlayersPage()
{
	for(let i = 0; i < players.length; i++)
	{
		...
		player.onclick = function(){switchPlayers(i)};
	}
}
...
function switchPlayers(playerNum)
{
	...
	var games = Math.round(player.games_played);
	var avgPassing = Math.round(player.pass_yards / games);
	var avgRushing = Math.round(player.rushing_yards / games);
	var avgReceiving = Math.round(player.receiving_yards / games);
